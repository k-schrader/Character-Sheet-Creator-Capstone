...
public class PageController {
	...
	// Injects instances of each needed repository so that we can use them inside of
	// this controller
	@Autowired
	...
	/*
	 * Handles requests to the first page of the character creator. If no charId is
	 * found, this method creates a new CharacterSheet and adds it to the model
	 * along with all the races and classes. If a charId is found, the method
	 * retrieves the CharacterSheet by id and, if the user is authenticated to view
	 * it, adds all the same information and allows the user to update an existing
	 * sheet. If not authorized, shows the unauthorized page
	 */
	...
	/*
	 * Adds the half finished character sheet from page one to a saved sheet to pass
	 * through to the second page to be finished. Redirects to the second page with
	 * the charId for the saved CharacterSheet as the endpoint
	 */
	@PostMapping("/page1")
	...
	/*
	 * Handles requests to the second page of the character creator Adds the items
	 * and spells for the views to access If-else added to assure that only the same
	 * user from page one can access page 2 This allows us to both edit the
	 * CharacterSheet if the user selects through to the edit page feature, and acts
	 * as the initial CharacterSheet builder
	 */
	...
	public String submit(@PathVariable("charId") Integer charId, Model model, Authentication authentication) {
		CharacterSheet characterSheet = charSheetRepo.findByCharId(charId); // Gets characterSheet with id passed in by
																			// page1 controller's redirect which adds
																			// our half finished character sheet values
																			// to the view
		if (authentication.getName().equals(characterSheet.getUsername())) {
			model.addAttribute("characterSheet", characterSheet);
			...
			for (Spells spell : spellsRepo.findAll()) {
				if (spell.getCastingClasses().contains(className)) {
					switch (spell.getCastingLevel()) { // Assess the spells casting level to separate into spells vs
														// cantrips
					case 0:
	...
	/*
	 * Handles requests to the final character sheet page If the user is
	 * authenticated they are passed on to their character sheet page New formatting
	 * service created to use on the view page
	 */
	...
	/*
	 * Handles requests to view a list of all a users old character sheets Checks
	 * first if user is the same as the old sheet user, if else they are redirected
	...
	/*
	 * This method deletes an existing character sheet It finds the sheet specified
	 * by the charId path variable and, if the user is authenticated, deletes the
	 * character sheet It then redirects the user back to the oldcharacterview page
	 */
